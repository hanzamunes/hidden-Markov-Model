...
public class WaveData {
	...
	public void saveToFile(String name, AudioFileFormat.Type fileType, AudioInputStream audioInputStream) {
		...
		JFileChooser jf = new JFileChooser(){
		    ...
		    public void approveSelection(){
				...
				if (FilenameUtils.getExtension(f.getName()).equalsIgnoreCase("wav")) {
					...
					//myFile = new File(f.toString() + ".wav");  // append .wav if "foo.jpg.wav" is OK
					f = new File(f.getParentFile(), FilenameUtils.getBaseName(f.getName())+".wav"); // ALTERNATIVELY: remove the extension (if any) and replace it with ".wav"
				}
		...
		jf.setFileFilter(new WavFileFilter());
		jf.setAcceptAllFileFilterUsed(false);
		int rVal = jf.showSaveDialog(null);
		if (rVal == JFileChooser.APPROVE_OPTION)
		{
			File myFile = jf.getSelectedFile();
			if (FilenameUtils.getExtension(myFile.getName()).equalsIgnoreCase("wav")) {
			    // filename is OK as-is
			} else {
				//myFile = new File(myFile.toString() + ".wav");  // append .wav if "foo.jpg.wav" is OK
				myFile = new File(myFile.getParentFile(), FilenameUtils.getBaseName(myFile.getName())+".wav"); // ALTERNATIVELY: remove the extension (if any) and replace it with ".wav"
			}
			try {
